...
public class WorkspaceRepository : IWorkspaceRepository
{
  ...
  public async Task<List<Workspace>> GetWorkspacesByIdsAsync(List<Guid> workspaceIds, string userId)
  {
    return await _context.Workspaces
      .Where(w => workspaceIds.Contains(w.Id) && w.UserId == userId)
      .ToListAsync();
  }

  public async Task UpdateRangeAsync(List<Workspace> workspaces)
  {
    _context.Workspaces.UpdateRange(workspaces);
    await _context.SaveChangesAsync();
  }
}