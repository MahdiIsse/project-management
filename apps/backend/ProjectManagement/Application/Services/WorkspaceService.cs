...
public class WorkspaceService : IWorkspaceService
{
  ...
  public async Task UpdateWorkspacePositionsAsync(List<UpdateWorkspacePositionDto> updates, string userId)
  {
    var workspaceIds = updates.Select(u => u.Id).ToList();
    var workspaces = await _workspaceRepository.GetWorkspacesByIdsAsync(workspaceIds, userId);

    if (workspaces.Count != updates.Count)
    {
      throw new NotFoundException("One or more workspaces not found or access denied");
    }

    foreach (var update in updates)
    {
      var workspace = workspaces.First(w => w.Id == update.Id);
      workspace.SetPosition(update.Position);
    }

    await _workspaceRepository.UpdateRangeAsync(workspaces);
  }
}