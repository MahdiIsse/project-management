...
public class ProjectTaskService : IProjectTaskService
{
  ...
  public async Task<ProjectTaskDto> UpdateAsync(Guid taskId, UpdateProjectTaskDto updateProjectTaskDto, string userId)
  {
    ...
    if (task.ColumnId != updateProjectTaskDto.ColumnId)
    {
      task.MoveToColumn(updateProjectTaskDto.ColumnId, updateProjectTaskDto.Position);
    }
    else
    {
  ...
  public async Task UpdateTaskPositionsAsync(Guid workspaceId, List<UpdateTaskPositionDto> updates, string userId)
  {
    var workspace = await _workspaceRepository.GetByIdAsync(workspaceId);
    if (workspace == null || !await _workspaceRepository.UserHasAccessToWorkspaceAsync(workspaceId, userId))
    {
      throw new NotFoundException("Workspace not found or access denied");
    }

    var taskIds = updates.Select(u => u.Id).ToList();
    var tasks = await _projectTaskRepository.GetTasksByIdsAsync(taskIds);

    if (tasks.Count != updates.Count)
    {
      throw new NotFoundException("One or more tasks not found");
    }

    var invalidTasks = tasks.Where(t => t.WorkspaceId != workspaceId).ToList();
    if (invalidTasks.Any())
    {
      throw new ForbiddenException("One or more tasks do not belong to the specified workspace");
    }

    foreach (var update in updates)
    {
      var task = tasks.First(t => t.Id == update.Id);
      task.MoveToColumn(update.ColumnId, update.Position);
    }

    await _projectTaskRepository.UpdateRangeAsync(tasks);
  }
}