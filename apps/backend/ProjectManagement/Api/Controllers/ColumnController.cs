...
public class ColumnController : BaseController
{
  ...
  [HttpPatch("positions")]
  public async Task<IActionResult> UpdateColumnPositions(
    [FromRoute] Guid workspaceId,
    [FromBody] List<UpdateColumnPositionDto> updates
  )
  {
    var userId = GetCurrentUserId();
    if (userId == null)
    {
      _logger.LogWarning("Unauthorized attempt to update column positions in workspace {WorkspaceId}", workspaceId);
      return Unauthorized();
    }

    _logger.LogInformation("Updating {Count} column positions in workspace {WorkspaceId} for user {UserId}", updates.Count, workspaceId, userId);

    try
    {
      await _columnService.UpdateColumnPositionsAsync(workspaceId, updates, userId);

      _logger.LogInformation("Successfully updated {Count} column positions in workspace {WorkspaceId}", updates.Count, workspaceId);

      return NoContent();
    }
    catch (Exception ex)
    {
      _logger.LogError(ex, "Failed to update column positions in workspace {WorkspaceId}", workspaceId);
      throw;
    }
  }
}