...
public class TasksController : BaseController
{
  ...
  [HttpPatch("positions")]
  public async Task<IActionResult> UpdateTaskPositions(
    [FromRoute] Guid workspaceId,
    [FromBody] List<UpdateTaskPositionDto> updates
  )
  {
    var userId = GetCurrentUserId();
    if (userId == null)
    {
      _logger.LogWarning("Unauthorized attempt to update task positions in workspace {WorkspaceId}", workspaceId);
      return Unauthorized();
    }

    _logger.LogInformation("Updating {Count} task positions in workspace {WorkspaceId} for user {UserId}", updates.Count, workspaceId, userId);

    try
    {
      await _projectTaskService.UpdateTaskPositionsAsync(workspaceId, updates, userId);

      _logger.LogInformation("Successfully updated {Count} task positions in workspace {WorkspaceId}", updates.Count, workspaceId);

      return NoContent();
    }
    catch (Exception ex)
    {
      _logger.LogError(ex, "Failed to update task positions in workspace {WorkspaceId}", workspaceId);
      throw;
    }
  }
}