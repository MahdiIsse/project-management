...
public class WorkspaceController : BaseController
{
  ...
  [HttpPatch("positions")]
  public async Task<IActionResult> UpdateWorkspacePositions(
    [FromBody] List<UpdateWorkspacePositionDto> updates
  )
  {
    var userId = GetCurrentUserId();
    if (userId == null)
    {
      _logger.LogWarning("Unauthorized attempt to update workspace positions");
      return Unauthorized();
    }

    _logger.LogInformation("Updating {Count} workspace positions for user {UserId}", updates.Count, userId);

    try
    {
      await _workspaceService.UpdateWorkspacePositionsAsync(updates, userId);

      _logger.LogInformation("Successfully updated {Count} workspace positions", updates.Count);

      return NoContent();
    }
    catch (Exception ex)
    {
      _logger.LogError(ex, "Failed to update workspace positions");
      throw;
    }
  }
}