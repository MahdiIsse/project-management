...
import { Task, priorityOptions } from "../..";
...
export function TaskForm({
  ...
  const { mutate: updateTask, isPending: isUpdatePending } = useUpdateTask(workspaceId);
  const { mutate: createTask, isPending: isCreatePending } = useCreateTask(workspaceId);
  ...
  const onSubmit = (data: TaskSchemaValues) => {
    ...
    if (isEditMode) {
      ...
      createTask({ 
        columnId: processedData.columnId,
        data: processedData,
      });
      closeDialog();
    }
                  ...
                  <SelectContent>
                    {columns?.map((column: Column) => (
                      <SelectItem
                        ...
                        disabled={isPending}
                      >
                        <span className={getColumnTextStyle(column.color || undefined)}>
                          {column.title}
                        </span>
                      </SelectItem>
                    ))}
